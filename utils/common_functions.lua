...
-- Ensures asset keys are generated consistently.
-- Example: "folder/subfolder/image.png" becomes "subfolder/image"
-- Example: "image.png" becomes "image"
function CommonFunctions.get_key_from_path(filepath_in_assets)
    print("CommonFunctions.get_key_from_path called with: " .. filepath_in_assets)
    local parts = {}
    -- Original gmatch pattern from main.lua, robust for typical asset paths.
    for part in string.gmatch(filepath_in_assets, "[^/\\]+") do table.insert(parts, part) end

    local key
    if #parts >= 2 then
        local name_ext = parts[#parts] -- e.g., "image.png"
        local folder = parts[#parts-1] -- e.g., "subfolder"
        local name = name_ext:match("(.+)%..+$") or name_ext -- Removes extension, e.g., "image"
        key = folder .. "/" .. name -- e.g., "subfolder/image"
    elseif #parts == 1 then
        local name_ext = parts[1] -- e.g., "image.png"
        key = name_ext:match("(.+)%..+$") or name_ext -- Removes extension, e.g., "image"
    else
        key = filepath_in_assets -- Fallback, though unlikely for valid asset paths
    end
    -- print("Generated asset key: '" .. key .. "' from path: '" .. filepath_in_assets .. "'") -- Optional: for more verbose tracing
    return key
end
